...
t_vec calc_view_direction(t_pos p0, t_line screen_line, int width, int sx)
{
    t_pos p1;
...
void draw_player_view(t_vars *vars, t_line screen_line)
{
    ...
    t_pos player_pos = (t_pos){player->x, player->y};
    ...
    while (sx < vars->img2.w)
    {
        ...
        direction = calc_view_direction(player_pos, screen_line, vars->img2.w, sx);
        ...
        line_length = calculate_line_length(player_pos, (player->angle + 90) % 360, cross_point);
        if (line_length > 0)
